...
void android_vh_do_send_sig_handler(void *unused, int sig, struct task_struct *cur, struct task_struct *p)
{
	...
	int is_32bit;
	struct cpumask *boost_mask = (struct cpumask *)cpu_possible_mask;
		...
		do {
			...
			if (!is_32bit) {
				if (!cpumask_empty(boost_mask)) {
					cpumask_copy(&thread->cpus_mask, boost_mask);
					thread->cpus_ptr = boost_mask;
					thread->nr_cpus_allowed = cpumask_weight(boost_mask);
				}
			}
