...
/*
 * TODO: using frameworks config xml instead of this function.
 */
#define PROC_TAOBAO          ("m.taobao.taobao")
#define PROC_TAOBAO_WEBVIEW  ("vilege_process0")

#define THD_CRRENDERERMAIN  ("CrRendererMain")
#define THD_COMPOSITOR      ("Compositor")

static bool user_config_frame_task(struct task_struct *p, const char *comm)
{
	...
	if (strncmp(leader->comm, PROC_TAOBAO, TASK_COMM_LEN)
		&& strncmp(leader->comm, PROC_TAOBAO_WEBVIEW, TASK_COMM_LEN)) {
		return false;
	}

	return (!strncmp(comm, THD_CRRENDERERMAIN, TASK_COMM_LEN)
			|| !strncmp(comm, THD_COMPOSITOR, TASK_COMM_LEN));
}

static inline bool check_set_frame_task(struct task_struct *p)
{
	...
	return css ? (css->id == CGROUP_TOP_APP) : false;
}

bool add_rm_related_frame_task(struct task_struct *tsk, int pid, int tid, int add, int r_depth, int r_width)
{
	unsigned long flags;
	struct frame_group *grp = NULL;
	bool success = false;
...
void fbg_task_rename_hook(void *unused, struct task_struct *p, const char *buf)
{
	char tmp_comm[TASK_COMM_LEN];
	bool is_top = false;

	strlcpy(tmp_comm, buf, TASK_COMM_LEN);
	is_top = check_set_frame_task(p);
	if (is_top && user_config_frame_task(p, tmp_comm)) {
		oplus_set_im_flag(p, IM_FLAG_WEBVIEW);
		add_rm_related_frame_task(p, p->tgid, p->pid, true, 0, 0);
	...
	unsigned int flags)
{
	...
	/* For composition group, vutil is useless */
	if (grp == &sf_composition_group)
		use_vutil = false;
...
static void fbg_cgroup_set_task_hook(void *unused, int ret, struct task_struct *p)
{
	...
	add_rm_related_frame_task(p, p->tgid, p->pid, is_top, 0, 0);
}
...
void register_frame_group_vendor_hooks(void)
{
	...
	register_trace_task_rename(fbg_task_rename_hook, NULL);
